MODULE cont2d
    ...
    TYPE cont2dsurf_type
        ...
        INTEGER(ipk), POINTER :: target_nodes(:)    ! Target nodes
        INTEGER(ipk) :: ncontactor ! Number of contactor nodes
        INTEGER(ipk), POINTER :: contactor_nodes(:) ! Contactor nodes
    ...
    TYPE cont2d_type
        ...
        REAL(r8k) :: gap0     ! Gap Function or shortest distance from contactor node to the target surface
        REAL(r8k) :: alpha    ! Element coordinate value at target surface at contact/shortest distance to contactor node
    ...
    DO WHILE (ASSOCIATED(current))
        IF (current%label == label) THEN
            WRITE (ounit,FMT='(A,I0)') 'ERROR two definitions for contact surface ',label
            lerror = .TRUE.
            CALL nlfemp_stop(0)
        END IF
    ...
    IF (numt < 2) THEN
        WRITE (ounit,FMT='(A,I0)') 'ERROR at least two target nodes needed for contact surface ',label
        lerror = .TRUE.
        CALL nlfemp_stop(0)
    END IF
    ...
    DO WHILE (ASSOCIATED(current_surf))
        DO i = 1, current_surf%ntarget - 1
            ...
            DO j = 1, current_surf%ncontactor
                ...
                IF ((nodes(3) == nodes(1)) .OR. (nodes(3) == nodes(2))) lactive = .FALSE.
                IF (lactive) THEN
                    ! Create new contact element If contactor node is close enough
                    ! to the target surface
                    label = ncont2d + 1
                    CALL cont2d_create (label, current_surf%mat, current_surf%label, nodes)
                ELSE
    ...
    IF (nfound /= 3) THEN
        WRITE (ounit,FMT='(/,A,/)') 'ERROR incompatible NODES and CONT2D data'
        lerror = .TRUE.
        CALL nlfemp_stop(0)
    END IF
    ...
    x12(1:2) = x(1:2,new_cont2d%node_numbers(2)) - x(1:2,new_cont2d%node_numbers(1))
    x13(1:2) = x(1:2,new_cont2d%node_numbers(3)) - x(1:2,new_cont2d%node_numbers(1))
    ...
    IF ((new_cont2d%alpha0 > -cont2d_alpha_tol) .AND. &
      ...
      & (new_cont2d%gap0 <= 0.0_r8k)) new_cont2d%closed0 = .TRUE.
    ...
    DO WHILE (ASSOCIATED(current))
        ...
        current%stick = .FALSE.
        IF ((alpha > -cont2d_alpha_tol) .AND. (alpha <  1.0_r8k + cont2d_alpha_tol) .AND. (gap <= 0.0_r8k)) THEN
            ...
            !Fint(1:2,current%node_numbers(1)) = Fint(1:2,current%node_numbers(1)) - Fn * (1.0_r8k - alpha0) * nv(1:2)
            !Fint(1:2,current%node_numbers(2)) = Fint(1:2,current%node_numbers(2)) - Fn * alpha0 * nv(1:2)
            !Fint(1:2,current%node_numbers(3)) = Fint(1:2,current%node_numbers(3)) + Fn * nv(1:2)

            Fext(1:2,current%node_numbers(1)) = Fext(1:2,current%node_numbers(1)) + Fn * (1.0_r8k -alpha0) * nv(1:2)
            Fext(1:2,current%node_numbers(2)) = Fext(1:2,current%node_numbers(2)) + Fn * alpha0 * nv(1:2)
            Fext(1:2,current%node_numbers(3)) = Fext(1:2,current%node_numbers(3)) - Fn * nv(1:2)
            ...
            IF (current%closed0) THEN
                ...
                !Fint(1:2,current%node_numbers(1)) = Fint(1:2,current%node_numbers(1)) - Ft * (1.0_r8k - alpha0) * tv(1:2)
                !Fint(1:2,current%node_numbers(2)) = Fint(1:2,current%node_numbers(2)) - Ft * alpha0 * tv(1:2)
                !Fint(1:2,current%node_numbers(3)) = Fint(1:2,current%node_numbers(3)) + Ft * tv(1:2)

                Fext(1:2,current%node_numbers(1)) = Fext(1:2,current%node_numbers(1)) + Ft * (1.0_r8k - alpha0) * tv(1:2)
                Fext(1:2,current%node_numbers(2)) = Fext(1:2,current%node_numbers(2)) + Ft * alpha0 * tv(1:2)
                Fext(1:2,current%node_numbers(3)) = Fext(1:2,current%node_numbers(3)) - Ft * tv(1:2)
            END IF
        END IF
        ...
        IF (current%closed) THEN
            ...
            IF (current%closed0) THEN
                IF (current%stick) THEN
                    CALL cont2d_stiff_t (x1, x2, x3, current%pfact, current%alpha0, current%l0, Ke)
                ELSE
                    symmetric_matrix = .FALSE.
                    frcoef = mat_par(current%mat,tref,'FRCOEF  ')
                    dir = SIGN(1.0_r8k, current%dalpha)
                    CALL cont2d_stiff_fr (x1, x2, x3, current%pfact, frcoef, dir, current%alpha0, Ke)
                END IF
    ...
    REAL(r8k), INTENT(IN) :: x1(2), x2(2), x3(2), pfact, alpha0
    REAL(r8k), INTENT(OUT) :: Ke(6,6)
    REAL(r8k) :: t1,   t2,   t5,   t6,   t9,  t10,  t11,  t12,  t13,  t14,  t15,  t16,  t18,  t19,  t20, &
      &         t23,  t24,  t25,  t29,  t30,  t36,  t37,  t38,  t39,  t43,  t44,  t48,  t51,  t52,  t54, &
      &         t55,  t58,  t59,  t62,  t65,  t67,  t71,  t73,  t74,  t78,  t79,  t80,  t86,  t93,  t96, &
      &        t100, t105, t110, t111, t117, t119, t126, t131, t136, t137, t143, t153, t158, t159, t161, &
      &        t165, t166
    ...
    t6 = x2(2) ** 2
    t9 = x1(2) ** 2
    t10 = t2 - 2.0_r8k * x2(1) * x1(1) + t5 + t6 - 2.0_r8k * x2(2) * x1(2) + t9
    t11 = SQRT(t10)
    t12 = 1.0_r8k / t11
    t13 = t1 * t12
    t14 = x3(1) - x1(1)
    ...
    t24 = t23 * t12
    t25 = -t23 * t19
    t29 = pfact * (-t13 - t15 * t20 / 2.0_r8k + t24 + t25 * t20 / 2.0_r8k)
    t30 = 1.0_r8k - alpha0
    ...
    t44 = t43 * t30
    t48 = -t36 * t19
    t51 = t29 * t44 - t36 * t12 * t30 - t48 * t39 / 2.0_r8k
    t52 = t13 * alpha0
    t54 = t18 * alpha0
    t55 = 2.0_r8k * t54 * t19
    t58 = -t29 * t52 + t37 * t55 / 2.0_r8k
    t59 = t43 * alpha0
    t62 = t36 * t12 * alpha0
    t65 = t29 * t59 - t62 - t48 * t55 / 2.0_r8k
    t67 = t1 * t18
    ...
    t79 = t14 * t12
    t80 = -2.0_r8k * t1 * t18
    t86 = pfact * (-t79 - t15 * t80 / 2.0_r8k + t43 + t25 * t80 / 2.0_r8k)
    t93 = -2.0_r8k * t54 * t1
    t96 = -t86 * t52 + t62 + t37 * t93 / 2.0_r8k
    t100 = t86 * t59 - t48 * t93 / 2.0_r8k
    t105 = t86 * t13 - t73 + t36 * t67 * t1
    t110 = -t86 * t43 - t36 * t74 * t1
    t111 = -2.0_r8k * t18 * t19
    t117 = pfact * (-t15 * t111 / 2.0_r8k - t24 + t25 * t111 / 2.0_r8k)
    t119 = -2.0_r8k * t54 * t19
    t126 = t117 * t59 + t62 - t48 * t119 / 2.0_r8k
    t131 = t117 * t13 + t36 * t67 * t19
    t136 = -t117 * t43 - t73 - t36 * t74 * t19
    t137 = 2.0_r8k * t1 * t18
    t143 = pfact * (t79 - t15 * t137 / 2.0_r8k + t25 * t137 / 2.0_r8k)
    ...
    t166 = t19 ** 2
    Ke(1,1) = -t29 * t13 * t30 + t37 * t39 / 2.0_r8k
    Ke(1,2) = t51
    Ke(1,3) = t58
    Ke(1,4) = t65
    Ke(1,5) = t71
    ...
    Ke(3,1) = t58
    Ke(3,2) = t96
    Ke(3,3) = -t117 * t52 + t37 * t119 / 2.0_r8k
    Ke(3,4) = t126
    Ke(3,5) = t131
    Ke(3,6) = t136
    Ke(4,1) = t65
    ...
    !
    REAL(r8k), INTENT(IN) :: x1(2), x2(2), x3(2), pfact, alpha0, l0
    REAL(r8k), INTENT(OUT) :: Ke(6,6)
    REAL(r8k) :: t1,   t2,   t5,   t6,   t9,  t11,  t12,  t13,  t14,  t15,  t16,  t17,  t19,  t20,  t23, &
      &         t24,  t25,  t32,  t33,  t37,  t38,  t39,  t44,  t45,  t47,  t48,  t49,  t50,  t55,  t56, &
      &         t58,  t62,  t63,  t66,  t67,  t71,  t72,  t77,  t79,  t81,  t85,  t87,  t92,  t93,  t96, &
      &        t104, t105, t113, t117, t122, t127, t132, t133, t143, t144, t146, t155, t160, t165, t166, &
      &        t176, t188, t193, t194, t196, t200, t201
    ...
    t6 = x2(2) ** 2
    t9 = x1(2) ** 2
    t11 = SQRT(t2 - 2.0_r8k * x2(1) * x1(1) + t5 + t6 - 2.0_r8k * x2(2) * x1(2) + t9)
    t12 = 1.0_r8k / t11
    t13 = t1 * t12
    t14 = x3(1) - x1(1)
    t15 = t14 * t12
    ...
    t32 = t16 * t12 + t25 * t12
    t33 = t32 * t19
    t37 = pfact * ((-t13 - t15 - t16 * t20 / 2.0_r8k - t25 * t20 / 2.0_r8k) * t12 + t33 * t1)
    t38 = t37 * l0
    t39 = 1.0_r8k - alpha0
    ...
    t56 = t55 * t39
    t58 = t24 * t19
    t62 = -t38 * t56 + t48 * t58 * t50 / 2.0_r8k
    t63 = t13 * alpha0
    t66 = t44 * t45 * alpha0
    t67 = -2.0_r8k * alpha0 * t1
    t71 = -t38 * t63 + t66 + t48 * t49 * t67 / 2.0_r8k
    t72 = t55 * alpha0
    t77 = -t38 * t72 + t48 * t58 * t67 / 2.0_r8k
    t79 = l0 * t1 * t12
    t81 = t44 * t45
    ...
    t93 = -2.0_r8k * t24 * t19
    t96 = t23 * t12
    t104 = pfact * ((-t16 * t93 / 2.0_r8k - t55 - t96 - t25 * t93 / 2.0_r8k) * t12 + t33 * t24)
    t105 = t104 * l0
    t113 = -2.0_r8k * alpha0 * t24
    t117 = -t105 * t63 + t48 * t49 * t113 / 2.0_r8k
    t122 = -t105 * t72 + t66 + t48 * t58 * t113 / 2.0_r8k
    t127 = t104 * t79 + t48 * t49 * t24
    t132 = t104 * t87 - t81 + t48 * t58 * t24
    t133 = 2.0_r8k * t19 * t1
    t143 = pfact * ((t15 - t16 * t133 / 2.0_r8k - t25 * t133 / 2.0_r8k) * t12 - t33 * t1)
    t144 = t143 * l0
    t146 = 2.0_r8k * alpha0 * t1
    t155 = -t144 * t72 + t48 * t58 * t146 / 2.0_r8k
    t160 = t143 * t79 + t81 - t48 * t49 * t1
    t165 = t143 * t87 - t48 * t58 * t1
    t166 = 2.0_r8k * t24 * t19
    t176 = pfact * ((-t16 * t166 / 2.0_r8k + t96 - t25 * t166 / 2.0_r8k) * t12 - t33 * t24)
    ...
    t201 = t24 ** 2
    Ke(1,1) = -t38 * t13 * t39 + t47 + t48 * t49 * t50 / 2.0_r8k
    Ke(1,2) = t62
    Ke(1,3) = t71
    Ke(1,4) = t77
    Ke(1,5) = t85
    ...
    Ke(3,1) = t71
    Ke(3,2) = t117
    Ke(3,3) = -t144 * t63 - t66 + t48 * t49 * t146 / 2.0_r8k
    Ke(3,4) = t155
    Ke(3,5) = t160
    Ke(3,6) = t165
    Ke(4,1) = t77
    ...
    !> CONT2D element matrix when there is tangential slipping
    !
    REAL(r8k), INTENT(IN) :: x1(2), x2(2), x3(2), pfact, frcoef, dir, alpha0
    REAL(r8k), INTENT(OUT) :: Ke(6,6)
    REAL(r8k) :: t1,   t2,   t3,   t4,   t7,   t8,  t11,  t12,  t13,  t14,  t16,  t17,  t18,  t20,  t21, &
      &         t22,  t25,  t26,  t27,  t30,  t31,  t32,  t33,  t38,  t39,  t41,  t42,  t43,  t44,  t45, &
      &         t50,  t51,  t57,  t58,  t61,  t66,  t71,  t72,  t75,  t80,  t85,  t86,  t89,  t94,  t95, &
      &         t98,  t99, t100, t103, t106, t111, t118, t125, t132, t133, t136, t140, t141, t148, t155, &
      &        t162, t169, t201, t202, t226, t232
    ...
    t8 = x2(2) ** 2
    t11 = x1(2) ** 2
    t12 = t4 - 2.0_r8k * x2(1) * x1(1) + t7 + t8 - 2.0_r8k * x2(2) * x1(2) + t11
    t13 = SQRT(t12)
    t14 = 1.0_r8k / t13
    t16 = x3(1) - x1(1)
    t17 = t16 * t3
    ...
    t26 = t25 * t14
    t27 = -t25 * t21
    t30 = -t3 * t14 - t17 * t22 / 2.0_r8k + t26 + t27 * t22 / 2.0_r8k
    t31 = -t30 * t21
    t32 = 1.0_r8k - alpha0
    t33 = t14 * t32
    t38 = t17 * t14 - t27 * t14
    ...
    t50 = t16 * t14
    t51 = -2.0_r8k * t20 * t3
    t57 = -t50 - t17 * t51 / 2.0_r8k - t21 * t14 + t27 * t51 / 2.0_r8k
    t58 = -t57 * t21
    t61 = -2.0_r8k * t32 * t3
    t66 = -2.0_r8k * t20 * t21
    t71 = -t17 * t66 / 2.0_r8k - t26 + t27 * t66 / 2.0_r8k
    t72 = -t71 * t21
    t75 = -2.0_r8k * t32 * t21
    t80 = 2.0_r8k * t20 * t3
    t85 = t50 - t17 * t80 / 2.0_r8k + t27 * t80 / 2.0_r8k
    t86 = -t85 * t21
    t89 = 2.0_r8k * t32 * t3
    t94 = 1.0_r8k / t12
    t95 = t3 * t94
    ...
    t136 = t14 * alpha0
    t140 = t2 * t39 * alpha0
    t141 = 2.0_r8k * alpha0 * t21
    t148 = -2.0_r8k * alpha0 * t3
    t155 = -2.0_r8k * alpha0 * t21
    t162 = 2.0_r8k * alpha0 * t3
    t169 = -t2 * t95 * alpha0 * t21
    t201 = t1 * t42 * t14
    t202 = -t38 * t21
    t226 = -t2 * t95 * t21
    t232 = t38 * t3
    Ke(1,1) = t2 * t31 * t33 - t41 - t43 * t44 * t45 / 2.0_r8k
    Ke(1,2) = t2 * t58 * t33 - t43 * t44 * t61 / 2.0_r8k
    Ke(1,3) = t2 * t72 * t33 + t41 - t43 * t44 * t75 / 2.0_r8k
    Ke(1,4) = t2 * t86 * t33 - t43 * t44 * t89 / 2.0_r8k
    Ke(1,5) = t98
    Ke(1,6) = -t2 * t100 * t32
    Ke(2,1) = t2 * t103 * t33 - t43 * t106 * t45 / 2.0_r8k
    Ke(2,2) = t2 * t111 * t33 - t41 - t43 * t106 * t61 / 2.0_r8k
    Ke(2,3) = t2 * t118 * t33 - t43 * t106 * t75 / 2.0_r8k
    Ke(2,4) = t2 * t125 * t33 + t41 - t43 * t106 * t89 / 2.0_r8k
    Ke(2,5) = t2 * t133 * t32
    Ke(2,6) = -t98
    Ke(3,1) = t2 * t31 * t136 - t140 - t43 * t44 * t141 / 2.0_r8k
    Ke(3,2) = t2 * t58 * t136 - t43 * t44 * t148 / 2.0_r8k
    Ke(3,3) = t2 * t72 * t136 + t140 - t43 * t44 * t155 / 2.0_r8k
    Ke(3,4) = t2 * t86 * t136 - t43 * t44 * t162 / 2.0_r8k
    Ke(3,5) = t169
    Ke(3,6) = -t2 * t100 * alpha0
    Ke(4,1) = t2 * t103 * t136 - t43 * t106 * t141 / 2.0_r8k
    Ke(4,2) = t2 * t111 * t136 - t140 - t43 * t106 * t148 / 2.0_r8k
    Ke(4,3) = t2 * t118 * t136 - t43 * t106 * t155 / 2.0_r8k
    Ke(4,4) = t2 * t125 * t136 + t140 - t43 * t106 * t162 / 2.0_r8k
    Ke(4,5) = t2 * t133 * alpha0
    Ke(4,6) = -t169
    Ke(5,1) = -t2 * t31 * t14 + t201 + t2 * t202 * t22 / 2.0_r8k
    Ke(5,2) = -t2 * t58 * t14 + t2 * t202 * t51 / 2.0_r8k
    Ke(5,3) = -t2 * t72 * t14 - t201 + t2 * t202 * t66 / 2.0_r8k
    Ke(5,4) = -t2 * t86 * t14 + t2 * t202 * t80 / 2.0_r8k
    Ke(5,5) = -t226
    Ke(5,6) = t1 * pfact * t99 * t94
    Ke(6,1) = -t2 * t103 * t14 + t2 * t232 * t22 / 2.0_r8k
    Ke(6,2) = -t2 * t111 * t14 + t201 + t2 * t232 * t51 / 2.0_r8k
    Ke(6,3) = -t2 * t118 * t14 + t2 * t232 * t66 / 2.0_r8k
    Ke(6,4) = -t2 * t125 * t14 - t201 + t2 * t232 * t80 / 2.0_r8k
    ...
    DO WHILE (ASSOCIATED(current))
        ...
        ELSE IF ((current%closed) .AND. (.NOT. current%stick)) THEN
            frcoef = mat_par(current%mat,tref,'FRCOEF  ')
            current%alpha0 = current%alpha + SIGN(1.0_r8k,current%dalpha) * frcoef * current%gap
        END IF
    ...
    DO WHILE (ASSOCIATED(current_surf))
        ...
        WRITE (UNIT=nunit) current_surf%contactor_nodes(1:current_surf%ncontactor)
        current_surf => current_surf%next
    END DO
    ...
    DO i = 1, ncont2dsurf
        IF (.NOT. ASSOCIATED(current_surf)) THEN
            ...
            END IF
        END IF
        IF (ASSOCIATED(current_surf%target_nodes)) DEALLOCATE(current_surf%target_nodes)
        IF (ASSOCIATED(current_surf%contactor_nodes)) DEALLOCATE(current_surf%contactor_nodes)
        ...
        READ(UNIT=nunit,ERR=20,END=20) current_surf%target_nodes(1:current_surf%ntarget)
        READ(UNIT=nunit,ERR=20,END=20) current_surf%ncontactor
        ALLOCATE(current_surf%contactor_nodes(current_surf%ncontactor))
        READ(UNIT=nunit,ERR=20,END=20) current_surf%contactor_nodes(1:current_surf%ncontactor)
        last_cont2dsurf => current_surf
        current_surf => current_surf%next
    END DO
...
20  CONTINUE
    WRITE (ounit,FMT='(/,A,/)') 'ERROR while reading CONT2D database'
    lerror = .TRUE.
    CALL nlfemp_stop(0)
