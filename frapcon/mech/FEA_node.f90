MODULE FEA_Node_frapcon
    USE Kinds_frapcon
    USE conversions_frapcon
    IMPLICIT NONE
    !>@brief
    !> This module contains the subroutine used to create a node_frapcon.
    ...
    DO WHILE (ASSOCIATED(current))
        IF (current%label == label) THEN
            WRITE (ounit,FMT='(A,I0,A)') 'ERROR: node ',label,' defined twice'
            lerror = .TRUE.
            CALL nlfemp_stop(0)
        END IF
