MODULE solid1D
    ...
    TYPE solid1d_type
        ...
        LOGICAL :: rigid       ! If true there will be no stress strain analysis in this element type
    ...
    IF (nfound /= 4) THEN
        WRITE (ounit,FMT='(/,A,/)') 'ERROR incompatible NODES and SOLID1D data'
        lerror = .TRUE.
        CALL nlfemp_stop(0)
    END IF
    ...
    DO WHILE (ASSOCIATED(current))
        IF (current%label == label) THEN
            WRITE (ounit,FMT='(/,A,I0,A,/)') 'ERROR SOLID1D ',label,' has been defined twice'
            lerror = .TRUE.
            CALL nlfemp_stop(0)
        END IF
    ...
    ! Initial derivatives
    CALL solid1d_deriv(x0(1,new_solid1d%node_numbers(1)), x0(1,new_solid1d%node_numbers(2)), &
      &                x0(1,new_solid1d%node_numbers(3)), x0(1,new_solid1d%node_numbers(4)), &
      &                new_solid1d%dV0, new_solid1d%dNdx, new_solid1d%x0)
    ...
    DO WHILE (ASSOCIATED(current))
        ...
        IF (current%rigid) THEN
            DO i = 1, 4
                IF (dofnum(i) > 0) CALL sparse_matrix_add_member(dofnum(i),dofnum(i))
            END DO
        ELSE
    ...
    DO WHILE (ASSOCIATED(current))
        IF (.NOT. current%rigid) THEN
            ...
            gtemp = 0.5_r8k * (temp(current%node_numbers(1)) + temp(current%node_numbers(2)))
            current%gtemp = gtemp
    ...
    REAL(r8k), POINTER :: gtemp, mu, lambda, epsth, epsel(:), epseff, epseff0, &
            gamma, Je, epspl(:), epspl0(:), phyd, sigma(:), sigeff, epstot(:), av(:), &
            dplmod, kappa, taueff, sigdev(:), deds
    ...
    DO WHILE (ASSOCIATED(current))
        IF (.NOT. current%rigid) THEN
            ...
            ! Derivatives
            CALL solid1d_deriv(x(1,nodes(1)), x(1,nodes(2)), x(1,nodes(3)), x(1,nodes(4)), &
              &                current%dV, current%dNdx, current%x)

            ! RETURN If there are poorly shaped elements
            IF (lerror) THEN
                WRITE (ounit,*) 'Error in subroutine solid1d_stress after calling solid1d_deriv.'
                RETURN
            END IF
            ...
            ! Principal stretches
            dl(1) = 1.0_r8k + (u(1,nodes(2)) - u(1,nodes(1))) / (x0(1,nodes(2)) - x0(1,nodes(1)))
            dl(2) = 1.0_r8k + (u(2,nodes(4)) - u(2,nodes(3))) / (x0(2,nodes(4)) - x0(2,nodes(3)))
            dl(3) = 1.0_r8k + (u(1,nodes(1)) + u(1,nodes(2))) / (x0(1,nodes(1)) + x0(1,nodes(2)))
            ...
            IF (lerror) THEN
                WRITE (ounit,*) 'Error in subroutine solid1d_stress after calling mat_creep_calc.'
                RETURN
            END IF
            ...
            IF (lerror) THEN
                WRITE (ounit,*) 'Error in subroutine solid1d_stress after calling mat_radial_return.'
                RETURN
            END IF
            ...
            IF (gamma > 0.0_r8k) THEN
                ...
                delta_plastic_strain_energy = delta_plastic_strain_energy + gamma * sigeff * current%dV
            ...
            ! Update energies
            elastic_strain_energy = elastic_strain_energy + (mu * (epsel(1) ** 2 + epsel(2) ** 2 + epsel(3) ** 2) &
    ...
    !
    TYPE(solid1d_type), POINTER :: current
    REAL(r8k), POINTER :: sigma(:), dNdx(:), dV
    ...
    DO WHILE (ASSOCIATED(current))
        IF (.NOT. current%rigid) THEN
            ...
            Fint(1,current%node_numbers(2)) = Fint(1,current%node_numbers(2)) + &
                                              (dNdx(1) * sigma(1) + sigma(3) * dNdx(3)) * dV
            Fint(2,current%node_numbers(3)) = Fint(2,current%node_numbers(3)) - dNdx(2) * sigma(2) * dV
            Fint(2,current%node_numbers(4)) = Fint(2,current%node_numbers(4)) + dNdx(2) * sigma(2) * dV
        END IF
        current => current%next
    END DO
    ...
    DO WHILE (ASSOCIATED(current))
        ...
        ELSE
            ...
            Ke(1,1) = Ke(1,1) + (dNdx(1) * sigma(1) * dNdx(1) + dNdx(3) * sigma(3) * dNdx(3)) * dV
            Ke(2,2) = Ke(2,2) + dNdx(2) * sigma(2) * dNdx(2) * dV
            Ke(1,3) = Ke(1,3) + (-dNdx(1) * sigma(1) * dNdx(1) + dNdx(3) * sigma(3) * dNdx(3)) * dV
            Ke(3,1) = Ke(1,3)
            Ke(3,3) = Ke(3,3) + (dNdx(1) * sigma(1) * dNdx(1) + dNdx(3) * sigma(3) * dNdx(3)) * dV
    ...
    !
    INTEGER(ipk) :: i,k
    REAL(r8k) :: Je, mu, lambda, kappa, sigdev(3), phyd, gamma, taueff, av(3), &
      &          dplmod, deds, tmod(6), aik(3,3), bik, tmp(6), tmp2(3,3), t2, &
      &          t4, t7, t9, t12, t15, t16, t21, t22, t26, t27, t40, t44, t48, t57
    ...
    IF (gamma > 1.0e-10_r8k) THEN
        IF (deds > 0.0_r8k) THEN
            ...
            t9 = tmp(3) * tmp(6)
            t12 = tmp(6) ** 2
            t15 = 1.0_r8k / (t4 * tmp(4) - tmp(1) * t2 - t7 * tmp(4) + 2 * t9 * tmp(5) - t12 * tmp(2))
    ...
    current => first_solid1d
    DO WHILE (ASSOCIATED(current))
        current%plSED = current%plSED + 0.5_r8k * current%gamma * (current%sigeff0 + current%sigeff)
...
20  CONTINUE
    WRITE (ounit,FMT='(/,A,/)') 'ERROR while reading SOLID1D database'
    lerror = .TRUE.
    CALL nlfemp_stop(0)
