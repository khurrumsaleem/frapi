MODULE math
    ...
    t12 = A(1,2) * A(3,1)
    t14 = A(1,3) * A(3,1)
    t17 = 1.0_r8k / (t4 * A(3,3) - t6 * A(3,2) - t8 * A(3,3) + t10 * A(3,2) + t12 * A(2,3) - t14 * A(2,2))
    t18 = t3 * t17
    ...
    DO
        ...
        CASE (1)
            ...
            s2 = s ** 2
            cs = c * s
            tmp2(1) = c2 * tmp(1) - 2.0_r8k * cs * tmp(4) + s2 * tmp(2)
            tmp2(2) = s2 * tmp(1) + 2.0_r8k * cs * tmp(4) + c2 * tmp(2)
            tmp2(3) = tmp(3)
