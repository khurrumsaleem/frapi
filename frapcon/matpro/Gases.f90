MODULE Gas
    ...
    REAL(r8k), PARAMETER :: airMWt = 0.78084_r8k * N2MWt + 0.20946 * O2MWt + 9.34E-3_r8k * ArMWt + &
      &                              3.3E-4_r8k * CO2MWt + 2.0E-5 + 1.0E-5
    ...
    CASE ('THERMCOND')       ! Thermal Conductivity
        mat_prop = gthcon (temp)
    CASE ('SPECHEAT')        ! Specific Heat
        mat_prop = GasCP (temp)
    CASE ('VISCOSITY')       ! Viscosity
        mat_prop = gvisco (temp)
    CASE ('DENSITY')         ! Density
        mat_prop = GasRho (temp)
...
100     FORMAT ('Error in Module Gas. Invalid material property ID. Material Property ID = ',a20)
    ...
    ! Input
    !
    ! gases(i) - mole fractions of the gas mixture.
    ! gtempK   - gas temperature (k)
    ! gpres    - gas pressure (psi) used for knudsen domain correction and for steam
    !
    ! Output
    !
    ...
    !
    INTEGER(ipk) :: i, j
    REAL(r8k) :: gtempC, gpres, fij, gij, kmix, rmoles, sum, rMOLES1
    REAL(r8k), INTENT(IN) :: gtempk
    ...
    IF (gtempK < tvLimitMin .OR. gtempK > tvLimitMax .OR. MAXVAL(gases) <= MinMoles) THEN
...
180     FORMAT (' Error in gthcon. Bad input. gtempK= ',e12.4,' K',/,' Gas Fractions ',7(e12.4,3x))
    ...
    DO i = 1, ngases
        ...
        CASE (SteamIndex)
            ! Steam
            gtempC = MAX(0.1_r8k, tkc(gtempk))
            gpres = press * PsitoPa
            IF (gtempK <= 973.15_r8k) THEN
                kgas(i) = gpres / gtempK * (-2.8516e-8_r8k + 9.424e-10_r8k * gtempK - 6.005e-14_r8k * gtempK ** 2) &
                  &      + 1.009_r8k * gpres ** 2 / ((gtempK ** 2) * (gtempC ** 4.2_r8k)) + (17.6e-4_r8k + 5.87e-5_r8k * (gtempC) &
                  &      + 1.08e-7_r8k * (gtempC) ** 2 - 4.51e-11_r8k * (gtempC) ** 3)
            ELSE
                kgas(i) = 4.44e-6_r8k * gtempK ** 1.45_r8k + 9.45e-5_r8k * (2.1668e-9_r8k * gpres / gtempK) ** 1.3_r8k
            END IF
        CASE (HeIndex, ArIndex, KrIndex, XeIndex, H2Index, N2Index, AirIndex)
            !
            ! Conductivity = aa * temp ^ bb
            kgas(i) = ncGasProperties(i)%KFit%aa * gtempK ** ncGasProperties(i)%KFit%bb
        CASE DEFAULT
            WRITE (0,101) i
            WRITE (ounit,101) i
101         FORMAT ('Error in subroutine gthcon. Wrong Gas ID called. GasID = ',i7)
    ...
    DO i = 1, ngases
        ...
        DO j = 1, ngases
            ...
            sum = sum + (1.0_r8k - KroneckerDelta(j)) * fij * gases(j)
        END DO
        kmix = kmix + kgas(i) * gases(i) / (gases(i) + sum)
    END DO
    !
    gthcon = kmix
    !
    ...
    ! Input
    !
    ! gases(i) - mole fractions of the gas mixture. The seven elements of gases must sum to 1.0
    !        1 = helium
    !        2 = argon
    !        3 = krypton
    !        4 = xenon
    ...
    ! Steam equation is from mayer, et al, "thermodynamic and transport properties of steam", 
    ! the american society of mechanical engineers (1967)
    !
    INTEGER(ipk) :: l, i, j
    INTEGER(ipk), DIMENSION(ngases) :: im = 1
...
180     FORMAT (' Error in gvisco2. Bad input. gtempK= ',e12.4,' K',/,' Gas Fractions ',7(e12.4,3x))
    ...
    DO i = 1, ngases
        ...
        CASE (SteamIndex)
            ...
            vgas(i) = (0.407_r8k * MAX(gtempK, 75.61_r8k) - 30.77_r8k) * 1.0E-7_r8k
        CASE (HeIndex, ArIndex, KrIndex, XeIndex, H2Index, N2Index, AirIndex)
            ! Viscosity = 2.6693E-6 * (MolWt * Temp) ^ 0.5 / (collision diameter ^2 * Omega)
            tkoe = MIN(gtempK / ncGasProperties(i)%mufit%eok, 400.0_r8k)
            omega = polate(Omegavk, tkoe, nt, im(i))
            vgas(i) = 2.6693E-6_r8k * SQRT(ncGasProperties(i)%MWt * gtempK) / ((ncGasProperties(i)%mufit%cld ** 2) * omega)
        CASE DEFAULT
            WRITE (0,101) i
            WRITE (ounit,101) i
101         FORMAT ('Error in subroutine gvisco. Wrong Gas ID called. GasID = ',i7)
    ...
    DO i = 1, ngases
        ...
        DO j = 1, ngases
            ...
            END IF
        END DO
        vmix = vmix + vgas(i) * gases(i) / (gases(i) + sum)
    END DO
    !
    gvisco = vmix
    !
...
180     FORMAT (' Error in gascp. Bad input. gtempK= ',e12.4,' K',/,' Gas Fractions ',7(e12.4,3x))
    ...
    DO i = 1, ngases
        ...
        CASE (HeIndex, ArIndex, KrIndex, XeIndex, H2Index, N2Index, AirIndex)
            IF (gtempK <= ncGasProperties(i)%cvFit%T0) THEN
                ! Ideal specific heat. cv = c0
                cvgas(i) = ncGasProperties(i)%cvFit%c0
            ELSE
                ! Non-ideal specific heat. cv = c0 + d0 * (temp - T0)
                cvgas(i) = ncGasProperties(i)%cvFit%c0 + ncGasProperties(i)%cvFit%d0 * (gtempK - ncGasProperties(i)%cvFit%T0)
            END IF
        CASE DEFAULT
            WRITE (0,101) i
            WRITE (ounit,101) i
101         FORMAT ('Error in subroutine GasCP. Wrong Gas ID called. GasID = ',i7)
    ...
    DO i = 1, ngases
        ...
        CASE (HeIndex, ArIndex, KrIndex, XeIndex, H2Index, N2Index, AirIndex)
            ...
            sum = gases(i) * (cvgas(i) + ncGasProperties(i)%Rgas)
        END SELECT
    ...
    IF (gases(SteamIndex) > MinMoles) THEN
        ! Steam Density
...
180     FORMAT (' Error in GasRho. Bad input. gtempK= ',e12.4,' K',/,' Gas Fractions ',7(e12.4,3x))
    ...
    x_tot = GasMwt ()
    DO i = 1, ngases
        x(i) = gases(i) * ncGasProperties(i)%MWt / x_tot
    END DO
    !
    ...
    ! Input
    !
    ! gases    - Array for storing the molar fraction of each gas species
...
180     FORMAT (' Error in GasMWt. Bad input. Gas Fractions ',7(e12.4,3x))
    ...
    GasMWt = 0.0_r8k
    DO i = 1, ngases
        MWt_i = ncGasProperties(i)%MWt * gases(i)
        GasMWt = GasMWt + MWt_i
    END DO
    !
    END FUNCTION GasMWt
    ...
    ! ngases - Number of gas species modeled
    !
    IF (.NOT. (ALLOCATED(gases))) THEN
        ALLOCATE(gases(1:ngases))
        gases = 0.0_r8k
    END IF
    !
