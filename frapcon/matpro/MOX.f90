MODULE MOX
    ...
    CASE ('THEXP')  ! Linear Thermal Expansion
...
100     FORMAT ('Error in Module MOX. Invalid material property ID. Material Property ID = ',a20)
    ...
    !> temperature, density, composition and burnup.
    !
    ! Input
    !
    ! burnup - Local burnup (MWd/MTU)
    ! comp   - PuO2 content of fuel (percent puo2 in total fuel weight)
    ! ftemp  - Fuel ring temperature (K)
    ...
    ! Output
    !
    ! con    - Fuel thermal conductivity (W/(m*K))
    !
    ! Reference:
    !
    ! Option number 1 (IMOX = 1)
    !
    ! The 100% dense solid MOX fuel thermal conductivity formulation is based on a combination of:
    ! (1) Duriez stoichiometry-dependent correlation, derived from diffusivity measurements on unirradiated fuel pellets
    !    (C.Duriez, et al, J.Nuclear Materials 277, 143-158 2000) 
    ! (2) the burnup degradation conatined in a modified version of the NFI fuel thermal conductivity model
    !
    ! Option number 2 (IMOX = 2)
    !
    ! The MOX fuel thermal conductivity formulation is based on:
    ! (1) OECD Halden Reactor Project report "Thermal Performance of High Burnup Fuel In-pile Temperature Data and Analysis"
    !     W.Wiesnack, T. Tverberg, Proceedings of the 2000 International Topical Meeting on LWR Fuel Performance
    !
    !
    ...
    SELECT CASE (imox)
    CASE (1)
        ! Using the Duriez/NFI Mod correlation combination
        ! base term for MOX where X = deviation from stoichiometry (2-O/M)
        fm = 1.0789_r8k * fden(j-1) / (1.0_r8k + 0.5_r8k * (1.0_r8k - fden(j-1)))
        ! fm is multiplied by 1.0789 to account for 95% TD
        ! Porosity correction is Lucuta correction, not Maxwell-Euken
        ! as proposed by Duriez et al.
        x = 2.0_r8k - fotmtl
        ax = 2.85_r8k * x + 0.035_r8k
        cx = (2.86_r8k - 7.15_r8k * x) * 1.0e-4_r8k
        !
        h = 1.0_r8k / (1.0_r8k + 396.0_r8k * EXP(-6380.0_r8k / ftemp))
        rphonon = 1.0_r8k / (ax + cx * ftemp + 0.00187_r8k * bug + 1.1599_r8k * gadoln(j-1) + &
          &      (1.0_r8k - 0.9_r8k * EXP(-0.04_r8k * bug)) * 0.038_r8k * (bug ** 0.28_r8k) * h)
        elect = (1.50e9_r8k / ftemp ** 2) * EXP(-13520.0_r8k / ftemp)
        base = rphonon + elect
        con = base * fm 
    CASE (2)
        ! Using the Halden correlation
        tc = tkc(ftemp)
        tco = MIN(1650.0_r8k, tc)
        buguo2 = bug * 0.8815_r8k
        fm = 1.0789_r8k * fden(j-1) / (1.0_r8k + 0.5_r8k * (1.0_r8k - fden(j-1)))
        base = 0.92_r8k / (0.1148_r8k + 0.004_r8k * buguo2 + 1.1599_r8k * gadoln(j-1) + &
          &    2.475e-4_r8k * (1.0_r8k - 0.00333_r8k * buguo2) * tco) + 0.0132_r8k * EXP(0.00188_r8k * tc)
        con = base * fm
    END SELECT
    ! Find uncertainty
    con = con * (1.0_r8k + ABS(sigftc * 0.088_r8k))
    !
    IF (ftemp <= 1800.0_r8k) THEN
        ucon = 0.07_r8k * con
    ELSE
        frac = (ftemp - 1800.0_r8k) / (3100.0_r8k - 1800.0_r8k) * (0.20_r8k - 0.07_r8k) + 0.07_r8k
        ucon = frac * con
    END IF
    !
    END FUNCTION fthcon
