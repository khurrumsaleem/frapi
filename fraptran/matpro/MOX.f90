...
!        mat_prop = ftmelt
    ...
    CASE ('THEXP')  ! Thermal Expansion
        mat_prop = UPuO2%fthexp ()
    CASE ('EMISS')  ! Emissivity
        mat_prop = UPuO2%femiss ()
    CASE ('ENTHALPY')  ! Enthalpy
        mat_prop = UPuO2%fenthl ()
...
100     FORMAT ('Error in Module mox_fraptran. Invalid material property ID. Material Property ID = ',a20)
    ...
    ! Input
    !
    ! UPuO2%Burnup  - Current local burnup (MWd/MTU)
    ! UPuO2%Temp    - Current fuel ring temperature (K)
    ! UPuO2%fraDen  - Fuel density (ratio of actual to theoretical)
    ! UPuO2%OMRatio - Oxygen to metal ratio of fuel (atoms oxygen/atoms metal)
    ! UPuO2%Gadoln  - Weight fraction of gadolinia in the fuel
    !
    ! Output
    !
    ! UPuO2%Thcon   - Fuel thermal conductivity (W/(m*K))
    !
    ! Reference:
    !
    ...
    ! Porosity correction is Lucuta correction, not Maxwell-Euken as proposed by Duriez et al.
    x = 2.0_r8k - UPuO2%OMRatio
    ax = 2.85_r8k * x + 0.035_r8k
    cx = (2.86_r8k - 7.15_r8k * x) * 1.0e-4_r8k
    h = 1.0_r8k / (1.0_r8k + 396.0_r8k * EXP(-6380.0_r8k / UPuO2%Temp))
    rphonon = 1.0_r8k / (ax + cx * UPuO2%Temp + 0.00187_r8k * bug + 1.1599_r8k * UPuO2%Gadoln + (1.0_r8k - &
      &       0.9_r8k * EXP(-0.04_r8k * bug)) * 0.038_r8k * bug ** 0.28_r8k * h)
    elect = (1.50e9_r8k / UPuO2%Temp ** 2) * EXP(-13520.0_r8k / UPuO2%Temp)
    base = rphonon + elect
    UPuO2%Thcon = base * fm 
    !
    ! Multiply by Uncertainty coefficient
    UPuO2%Thcon = UPuO2%Thcon * sigfuelthermcond
    con = UPuO2%Thcon
    !
    END FUNCTION fthcon
    !
    ...
    ERROR STOP 'Function: UPuO2Density in Module: MOX has not yet been implemented'
    ! UO2 (g/cm^3)
...
!        ftmelt = sldus(comp(j-1)) + 273.15_r8k - 5.0_r8k * fbu / 10000.0_r8k - 4.8_r8k * UPuO2%Gadoln(j-1) * 100.0_r8k
!        fdelta = liqdus(comp(j-1)) - sldus(comp(j-1)) - 5.0_r8k * fbu / 10000.0_r8k - 4.8_r8k * gadoln(j-1) * 100.0_r8k
!    ELSE ! UO2 Fuel
!        ftmelt = 3113.15_r8k - 5.0_r8k * fbu / 10000.0_r8k - 4.8_r8k * UPuO2%Gadoln(j-1) * 100.0_r8k
!        fdelta = 1.0e-10_r8k
!    END IF
    ! Heat of fusion
    fhefus = 27.4E4_r8k
    ...
    ! Input
    !
    ! UPuO2%Temp    - Temperature (k)
    ! facmot        - Fraction molten (unitless) (0.0 = all solid, 1.0 = all molten)
    ! UPuO2%OMRatio - Oxygen to metal ratio (unitless) 
    ...
    IF (UPuO2%Temp <= (tm + fdelta)) THEN
        !
        fcp = cp(c1u ,c2u ,c3u ,thu ,edu ,UPuO2%Temp) * (1.0_r8k - (UPuO2%wtPu / 100.0_r8k)) + &
          &   cp(c1pu,c2pu,c3pu,thpu,edpu,UPuO2%Temp) * (UPuO2%wtPu / 100.0_r8k)
        IF (UPuO2%Temp >= (tm - 0.1_r8k)) fcp = (1.0_r8k - UPuO2%facmot) * fcp + UPuO2%facmot * fcpmol
    ELSE
        fcp = fcpmol
        ufcp = 2.0_r8k
        pufcp = 5.6_r8k
    ...
    CONTAINS
        ...
        cp = c1 * (th ** 2) * EXP(th / t) / ((t ** 2) * ((EXP(th / t) - 1.0_r8k) ** 2)) + c2 * t + &
          &  (UPuO2%OMRatio / 2.0_r8k) * (c3 * ed / (8.314_r8k * (t ** 2))) * EXP(-ed / (8.314_r8k * t))
        !
        END FUNCTION cp
        !
    END FUNCTION fcp
    ...
    !
    IF (UPuO2%Temp <= ftmelt) THEN
        fthexp = ftx(c1u, c2u, c3u, edu, bk, UPuO2%Temp) * (1.0_r8k - (UPuO2%wtPu / 100.0_r8k)) + &
          &      ftx(c1pu, c2pu, c3pu, edpu, bk, UPuO2%Temp) * (UPuO2%wtPu / 100.0_r8k)
    ELSE
        fthexm = ftx(c1u, c2u, c3u, edu, bk, ftmelt) * (1.0_r8k - (UPuO2%wtPu / 100.0_r8k)) + &
          &      ftx(c1pu, c2pu, c3pu, edpu, bk, ftmelt) * (UPuO2%wtPu / 100.0_r8k)
        !
        IF (UPuO2%Temp <= (ftmelt + fdelta)) THEN
            fthexp = fthexm + 0.043_r8k * UPuO2%facmot
        ELSE
            fthexp = fthexm + 0.043_r8k + 3.6e-05_r8k * (UPuO2%Temp - (ftmelt + fdelta))
        ENDIF
    ENDIF
    ...
    !
    CLASS (MixedOxide), INTENT(IN):: UPuO2
    REAL(r8k) :: femisu, femisl
    !
    femiss = 0.78557_r8k + 1.5263e-05_r8k * UPuO2%Temp
    ...
    ! Input
    !
    ! UPuO2%facmot  - Fraction of molten fuel
    ! UPuO2%wtPu    - Weight percent PuO2 of the fuel
    ! UPuO2%hefus   - Heat of fussion of the fuel (J/kg)
    ! UPuO2%OMRatio - Fuel oxygen to metal ratio
    ! UPuO2%Tmelt   - Fuel melting temperature (K)
    ! UPuO2%Temp    - Local temperature (K)
    !
    ! Output
    !
    ...
    REAL(r8k), PARAMETER :: edpu = 1.967e05_r8k
    !
    tx = MIN(UPuO2%Temp, UPuO2%Tmelt)
    !
    IF (UPuO2%wtPu > 0.0_r8k) THEN !MOX
        fenthl = cpdt(c1u, thu, c2u, UPuO2%OMRatio, edu, tx, c3u) * (1.0_r8k - (UPuO2%wtPu / 100.0_r8k)) + &
          &      cpdt(c1pu, thpu, c2pu, UPuO2%OMRatio, edpu, tx, c3pu) * (UPuO2%wtPu / 100.0_r8k)
    ELSE !UO2
        fenthl = cpdt(c1u, thu, c2u, UPuO2%OMRatio, edu, tx, c3u) * (1.0_r8k - (UPuO2%wtPu / 100.0_r8k))
    ENDIF
    !
    IF (UPuO2%Temp > (UPuO2%Tmelt - 2.0_r8k)) THEN
        fenthl = fenthl + UPuO2%hefus * UPuO2%facmot
        IF (UPuO2%Temp > (UPuO2%Tmelt + 2.0_r8k)) fenthl = fenthl + (UPuO2%Temp - UPuO2%Tmelt) * fcpmol
    ENDIF
    !
    ! Multiply by Uncertainty coefficient
