...
\tikzset{%
  ...
  box0/.style = {base, minimum width=2.5cm, fill=green!15},
  box1/.style = {base, minimum width=2.5cm, fill=white!15, align=left},
...
\begin{frame}{Class ``FuelRod" Structure}
...
\begin{tikzpicture}[node distance=1.5cm, every node/.style={fill=white, font=\sffamily}, align=center]
  ...
  {Variables Decay \\ 
   ...
   Temperature \\
   ZrModels CladCreep};
  \node (onDriver_)    [box1, below of=onDriver,   yshift=-0.5cm]  
  {make : allocate vars \\ 
   proc : input process \\ 
  ...
  \node (onFuelrod_)   [box1, below of=onFuelrod,  yshift=-0.7cm]  
  {make   : set mesh size \\ 
